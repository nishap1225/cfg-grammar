...
class CFGSQLEvaluator:
    ...
    def __init__(self, query_generator: generate_query.QueryGenerator, query_db_client: query_db.QueryDB):
    ...
    def check_schema_match(self, actual_data: DataFrame, expected_columns: List) -> bool:
        """Check if returned data includes expected columns"""
        return expected_columns.issubset(set(actual_data.columns))
    
    def check_data_correctness(self, actual_data: DataFrame, expected_data: DataFrame) -> bool:
        """Check if actual data includes expected results"""
        for col in expected_data.columns:
            expected_vals = set(expected_data[col].dropna().unique()) # Get unique set of non-null values in epxected_data
    ...
    def evaluate_single_case(self, test_case: TestCase) -> EvalResult: #, use_cfg: bool) -> EvalResult:
        ...
        if not error_message:
            schema_matches = self.check_schema_match(actual_results, test_case.expected_columns)
        
        # Check data correctness
        data_correct = False
        if not error_message and schema_matches:
            data_correct = self.check_data_correctness(actual_results, test_case.expected_data)
    ...
    def run_evaluation(self, test_cases: List[TestCase]) -> Dict[str, Any]:
        ...
        def calculate_metrics(results: List[EvalResult]) -> Dict[str, float]:
            ...
            return {
                ...
                "schema_compliance_rate": sum(r.schema_matches for r in results) / total,
