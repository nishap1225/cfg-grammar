...
with tab2:
    ...
    for i, case in enumerate(test_cases):
        test_case_data.append({
            ...
            "Expected SQL": case.expected_sql[:100] + "..." if len(case.expected_sql) > 100 else case.expected_sql
        })
    ...
    if 'evaluation_results' in st.session_state:
        ...
        for i, result in enumerate(results['cfg_results']):
            with st.expander(f"Test {i+1}: {result.test_case.natural_language}"):
                ...
                if result.test_case.expected_data is not None and not result.test_case.expected_data.empty:
